...
1. **Hexagonal (Ports & Adapters), three layers, strictly ordered dependencies.** Backend code lives
   ...
   Enforced by **import-linter** in CI, not by good intentions. The `domain` contract is
   `forbidden`-type against *every* third-party package, expressed as a `layers` contract plus an
   `independence` contract, so a new dependency is caught by default rather than by a maintained
   allow-list.
...
4. `pytest` — all pass; new behaviour has `domain` unit tests **and** at least one
   application/API test against a real database.
