 ...
 * made.
 *
 * Shared by `JobPostingPanel` (which posting is on screen) and `TailorPanel` (which posting to
 * tailor against) for the reason `latestBaseCv`'s docstring gives: one rule, so the two panels can
 * never disagree about which posting is "the" posting.
 */
